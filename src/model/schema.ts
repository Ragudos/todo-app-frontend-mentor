import { pgTable, index, unique, pgEnum, text, timestamp, boolean } from "drizzle-orm/pg-core"
...
export const filterTodoValues = pgEnum("filter_todo_values", ['all', 'active', 'completed'])


export const todos = pgTable("todos", {
	id: text("id").$defaultFn(() => crypto.randomBytes(16).toString("hex")).primaryKey().notNull(),
	creationDate: timestamp("creation_date", { mode: 'string' }).defaultNow(),
	isFinished: boolean("is_finished").default(false),
	content: text("content").default('This todo does not have anything.').notNull(),
},
(table) => {
	return {
